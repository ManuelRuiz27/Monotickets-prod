...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (method === 'GET' && url.pathname === '/health') {
      return sendJson(res, 200, { status: 'ok', env: APP_ENV });
    }

    if (method === 'GET' && url.pathname.startsWith('/events/') && url.pathname.endsWith('/guests')) {
    ...
    if (method === 'POST' && url.pathname === '/scan/validate') {
      ...
      const hash = Array.from(`${code}:${eventId}`).reduce((acc, char) => acc + char.charCodeAt(0), 0);
    ...
    log({ level: 'error', message: 'request_failed', error: error.message, request_id: requestId, path: url.pathname });
  } finally {
    log({ level: 'info', message: 'request_completed', method, path: url.pathname, status: res.statusCode, request_id: requestId });
  }
});
...
async function ensureDependencies() {
  ...
  const redisHost = new URL(process.env.REDIS_URL || 'redis://redis:6379');
  const retries = Number(process.env.STARTUP_RETRIES || 10);
